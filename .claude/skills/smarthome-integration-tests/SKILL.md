...
```powershell
.\scripts\Run-IntegrationTests.ps1                              # WifiTest, MqttTest, BMP280Test
...
**The suite leaves the last test flashed on the device** (BMP280Test by default). Redeploy the
...
Tests run WiFi-first on purpose: `MqttTest` can only fail confusingly when the network itself is
broken. `MqttTest` targets a broker address hardcoded in its `Program.cs` — the script warns up
...
This is not `smarthome-test` — that one runs the `NFUnitTest` unit suite through
