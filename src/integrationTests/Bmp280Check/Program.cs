...
namespace SmartHome.IntegrationTests.Bmp280Check
{
    ...
    public class Program
    {
        ...
        private static void Run()
        {
            ...
            // Deliberately identical to RoomSensor.SetupSensor(), FilterMode included.
            // This test can only be the isolated proof that RoomSensor's sensor reads
            // correctly if it configures the sensor the way RoomSensor does. It used to
            // leave FilterMode unset here and then change all four settings mid-loop --
            // inherited from the upstream sample -- so the first measurement, the one
            // that decides the verdict, ran a configuration the shipped device never
            ...
            while (true)
            {
                ...
                // One read per iteration. The loop body was written out twice, the second
                // copy preceded by a reconfiguration of all four sampling settings --
                // both inherited verbatim from the upstream sample, and the
                // reconfiguration is what made the check stop measuring RoomSensor's
