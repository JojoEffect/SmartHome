...
namespace SmartHome.HomeAssistant
{
    ...
    /// that already carries several, to emit a few dozen members of flat,
    /// machine-generated JSON. The upstream <c>nanoFramework.Iot.Device.HomeAssistant</c>
    /// binding made the same call for the same reason (its <c>MiniJson</c>).
    ///
    ...
    internal sealed class JsonWriter
    {
        ...
        /// <c>NamedEntityBase.ValidateId</c> indexes.
        /// </remarks>
                ...
                switch (c)
                {
                    ...
                    default:
                        if (c < ' ')
                        {
                            ...
                            // above. Everything else -- including the degree sign of
                            // "°C" and any other non-ASCII -- is a legal JSON string
                            // character and goes out as UTF-8, which is what MQTT carries
                            // and what Home Assistant decodes.
                            _json.Append("\\u00");
