...
{
    /// <summary>
    /// A controller wrote to a settable property.
    /// </summary>
    /// <remarks>
    /// The property has already been updated by the time this is raised, and the new
    /// value has already been reflected back onto the property -- every convention this
    /// seam targets expects the device to publish the processed value as soon as it can.
    ///
    ...
    /// adapter's publish is already out by the time this is raised.
    ///
    /// This is the app's job rather than the library's on purpose -- see issue #33.
    ///
    /// Handlers run on the MQTT client's event-dispatch thread, the one thread that also
    /// carries SUBACK, PUBACK, UNSUBACK and the connection-closed signal. Don't block it
    /// waiting for hardware: hand slow work to another thread, return, and publish the
    /// outcome onto the property when it arrives.
    /// </remarks>
    public class DeviceCommandEventArgs : System.EventArgs
    {
        /// <remarks>
        /// Public, unlike the model's own event arguments, because the adapter that
        /// raises this lives in a different assembly: the seam is the whole point, so
        /// implementations are deliberately not privileged over anyone else here.
        /// </remarks>
        public DeviceCommandEventArgs(PropertyBase property, byte[] payload)
        {
            Property = property;
            Payload = payload;
        }
